...
function adoptNode(spec, el, css = true, ignoreKeys = []) {
    ...
    const reactiveProps = Object.entries(specDescriptors).filter(([key, descriptor]) => key.startsWith('$') && !ignoreKeys.includes(key));
            ...
            if (!(key in el)) {
                ...
                if (Signal.isState(existingProperty)) {
                    ...
                    if (typeof descriptor.value === 'function') {
                        ...
                        existingProperty.set(evalTemplate(descriptor.value, el));
                    }
                    else {
...
function define(elements) {
    ...
    unregisteredDDOMElements.forEach(spec => {
        ...
        const reactiveProps = Object.entries(specDescriptors).filter(([key, descriptor]) => key.startsWith('$'));
        // debug
